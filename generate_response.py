...
def _is_valid_turn_start(turn: str) -> bool:
    return turn.startswith(INTERLOCUTOR_A) \
...
def _is_alternating_interlocutors(dialogue: List[str]) -> bool:
    ...
    return all(
        role == role_expected
        for role, role_expected in
        zip(roles, cycle([INTERLOCUTOR_A, INTERLOCUTOR_B]))
    )
...
def load_dialogues(
    ...
    assert all(
        _is_valid_turn_start(turn)
        for dialogue in dialogues for turn in dialogue), \
    ...
    assert all(_is_alternating_interlocutors(dialogue[:-1] if has_cue else dialogue) for dialogue in dialogues)

    parsed_dialogues = [
        {
            'cue': remove_speaker_tag(dialogue[-1] if has_cue else ''),
            'history': list(map(remove_speaker_tag, dialogue[:-1] if has_cue else dialogue)),
        }
        for dialogue in dialogues
    ]

    return [
        prepare_instance(tokenizer, device, **dialogue)
